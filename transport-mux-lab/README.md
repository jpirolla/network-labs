...
5. **Latência de Processamento (`rate(sum) / rate(count)`):** Calcula a latência média entre o timestamp de envio (cliente) e o momento de recebimento (servidor). Latências negativas (valores < 0) ocorrem devido à limitação da metodologia de medição: timestamps capturados com `time.time()` em containers distintos podem apresentar pequenas variações (*jitter*) causadas pelo escalonamento do SO e pela resolução do timer, mesmo compartilhando o relógio do host. Isso não representa erro lógico, mas sim a granularidade do sistema de temporização. Em ambientes distribuídos reais, protocolos de sincronização (NTP/PTP) seriam necessários para eliminar esse jitter.
